...
class BaseModelManager(ABC):
        ...
        if os.path.exists(self.cache_file):
            ...
            try:
                with open(self.cache_file, "r") as f:
                    ...
                    self.image_embeddings = {k: np.array(v) for k, v in cached_data.items()}
                logger.info(f"✅ Loaded {len(self.image_embeddings)} cached {self.model_name} embeddings")
    ...
    async def compute_image_embeddings(self):
        ...
        image_files = [
            f for f in os.listdir(settings.IMAGES_PATH)
            if f.lower().endswith((".png", ".jpg", ".jpeg", ".avif", ".webp"))
        ]
        
        logger.info(f"🔄 Found {len(image_files)} images to process with {self.model_name}")
        ...
        for image_file in image_files:
            try:
                ...
                logger.warning(f"⚠️ Error processing {image_file} with {self.model_name}: {e}")
        
        # Save embeddings to cache
        await self.save_embeddings_cache()
        
        logger.info(f"✅ Computed and cached {len(self.image_embeddings)} {self.model_name} embeddings")
